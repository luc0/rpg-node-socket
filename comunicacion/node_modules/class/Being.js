exports.Being = function( params ){
		...
		if( this.controls == "pc" ){
			...
			function follow(){
				...
				if( destiny ){
					...
					if( movement && !movement.success ){ // si no pudo moverse -> colisiona.
			...
			this.timerCamina = timers.add( setInterval( follow.bind(this) ,330) );
	...
	// Mira donde crear el Being cuando se conecta el User o respawnea el NPC.
	this.setSpawnPosition = function(){
		var position;
		if( this.controls == 'pc'){
			position = { "x" : 1 , "y" : 1 }; // Por ahora es estatica, pero depende de el ultimo deslogueo.
		}else{
			position = this.npcSpawnPosition({ // Por ahora es estatica, pero cada NPC va a tener su area definida. (habitat)
				"from" : { "x" : 1 , "y" : 1 }, 
				"to" : { "x" : 29 , "y" : 29 } 
			});
		}
	...
	// Posicion random al crear NPC dentro de su habitat
	this.npcSpawnPosition = function( params ){

		var fromArea = params.from;
		var toArea = params.to;
		// Por ahora devuelve una posicion random de un mapa 30x30, mas adelante hacer que aparezca donde no haya un humano cerca y que sea dentro de "su zona"
		var xpos = Math.floor(Math.random()*toArea.x) + fromArea.x;
		var ypos = Math.floor(Math.random()*toArea.y) + fromArea.y;
	...
	this.move = function( params ){
		...
		// Si cambio de direccion, la envia al cliente
		if( this.prev_direction != this.direction ){
	...
	this.isAffected = function( params ){
		...
		for( var stat in stats){
			...
			// General
			if( stats[stat].min !== undefined) this.stats[stat].min += stats[stat].min;
			if( stats[stat].max !== undefined) this.stats[stat].max += stats[stat].max;
	...
	this.throw = function( params ){
		...
		if( params.clone){
			...
			//console.log("cantidad clonada ",clonedArtifact)
			world.getTile( params.position ).artifact = clonedArtifact;

			world.updateArtifact( { 'artifact' : clonedArtifact } );
			world.refreshTiles( {'element' : clonedArtifact} );
			return;
	...
	this.dead = function(){
		...
		if( this.controls == 'npc' ){
			...
			// reespawn
			var newId = world.generateId({ "prefix" : "b" });
			world.createBeing({ "userId" : newId , "type" : this.constructor , "controls" : "npc" });

			// remove
			timers.clear( this.timerCamina );
			this.remove();
		}
	...
	this.remove = function(){
		...
		world.updateWorldObjects({ "delete" : this }); // acutaliza createdObjects

		// A lo ultimo borro esta instancia.
		...
		if( !this.isEquipped({"artifact" : this.inventory[0]}) ){
			...
			if( this.inventory.length ){
				...
				if( !tileArtifact || tileArtifact.constructor == this.inventory[0].constructor ){
					...
					if(artifact_dropped.count == null) return false; // Error.

					// Si tiene mas de 1
					if( artifact_dropped.count > 1 ){
						// Y el user nunca dijo cuantos..
						if( params.dropAmount == null ){
							SERVER.askDrop({ "userId" : this.id });
							return false;
						}else{
						// Si ya dijo cuantos..
							// Se fija si quiere tirar mas de los que tiene, entonces tira todos..
							if( dropAmount > artifact_dropped.count ){
								dropAmount = artifact_dropped.count;
							}else{
	...
	*/
	...
	this.think = function( params ){
		...
		//console.log( message );
		//Hacer que se lo envie al usuario
	}
	...
	this.lifeEvent = function( params ){
		if( this.stats.life.min <=0 ){
			...
			this.dead();
			console.log(params.author.id+" MATO A "+this.id)
		}
	}
	...
	*/
	...
	// Being
	this.updateDirection = function( params ){ // Actualiza posicion del being
		SERVER.queue({
			"being" : this.id ,
			"controls" : this.controls,
			"data" : { "all" : [
