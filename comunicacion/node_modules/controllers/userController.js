...
exports.userController = function( socket ){
	test('alguien se quiere conectar');
	...
	function disconnect( params ){
		...
		if( disconnected_user ){
			...
			test( 'User desconectado.' );
		}else{
			console.log( 'Ya se había borrado' );
		}
	...
	function user_login( params ){
		...
		if( user ){
			if( USERS.connect({ "user" : user }) ){
				...
				var userId = user.username;//user.generateId();

				var data = { "userId" : userId };
				socket.emit( 'user_login' , data );

				// Envia data del world al cliente
				SERVER.initClient({ "socket" : socket , "userId" : userId }); 
			}
		}
