exports.Being = function( params ){
	...
	this.generateId = function(){
		var id;
		do{
			id = this.name + parseInt( Math.random() * 10000 );
	...
	this.createControls = function(){
		if( this.controls == "pc" ){
			...
			this.controls.eventsDown.up = (function(){
				...
				this.move({"y":-1})
				this.direction = 'up';
				world.refresh({ "element":this , "lastPosition":lastPosition });
			}).bind(this);
			...
			this.timerCamina = parseInt(setInterval( follow.bind(this) , 1000 ))
	...
	this.throw = function( params ){
		...
		// si Tile con artifacts iguales -> Suma cantidad
			...
			params.artifact.count += params.many;
		...
		this.updateArtifact({ 'artifact' : params.artifact });
		this.updateTile({ 'object' : params.artifact });
	...
	this.remove = function(){
		...
		this.updateWorldObjects(); // acutaliza createdObjects
		...
		if( !this.isEquipped({"artifact" : this.inventory[0]}) ){
			...
			if( this.inventory.length ){
				...
				if( !tileArtifact || tileArtifact.construct == this.inventory[0].construct ){
					...
					// Si tiene mas de 1, pregunta cantidad
					if( artifact_dropped.count > 1 ){
						dropAmount = + params.dropAmount || + Math.max( parseInt(prompt('Cantidad a tirar')) || 1 , 1 ); // params.dropAmount sirve para un futuro, lo otro pide un numero ( si no es mayor a 1 entonces lo toma como 1 igual)
					}
	...
	*/
	...
	this.experienceEvent = function(){
		if( this.stats.experience.min >= this.stats.experience.max ){
			...
			this.updateStat({ 'stat' : 'experience' });
			this.updateStat({ 'stat' : 'level' });
			this.updateStat({ 'stat' : 'damage' });
		}
	}
	...
	*/
	...
	// Being
	this.updateInventory = function(){ // Actualiza todo el inventario
		SERVER.queue({
			"being" : this.id ,
			"controls": this.controls,
			"data" : { "all" : [ 
				{ "tree" : ['inventory'] , "value" : this.inventory } 
			]}
		});
	}
	...
	this.updateState = function( params ){ // Actualiza un state del being
		...
		SERVER.queue({
			 ...
			 "controls" : this.controls,
			 "data" : { "all" : [ 
			 	{ "tree" : ['state',params.state] , "value" : this.state[ params.state ] } 
			 ]}
		});
	...
	this.updateStat = function( params ){ // Actualiza un stat del being
		SERVER.queue({
			 ...
			 "controls" : this.controls,
			 "data" : { "all" : [ 
			 	{ "tree" : ['stat',params.stat] , "value" : this.stats[ params.stat ] } 
			 ]}
		});
	...
	// Being
	this.updatePosition = function(){ // Actualiza posicion del being
		SERVER.queue({
			"being" : this.id ,
			"controls" : this.controls,
			"data" : { "all" : [ 
	...
	// Being
	this.updateEquipment = function(){ // Actualiza equipamento y modificadores
		SERVER.queue({
			"being" : this.id ,
			"controls" : this.controls,
			"data" : { "all" : [ 
	...
	this.updateModificators = function(){
		// Usarla para otras causas que no sean cuando equipas algo, porque esta hecho.
	}
	...
	// World
	this.updateWorldObjects = function(){ // Actualiza objetos instanciados en el world.
		SERVER.queue({
			 "world" : null ,
			 "data" : { "tree" : ['createdObjects'] , "value" : this }
		});
	}
