		...
		for( var stat in stats){
			...
			if( stat == 'life' ){ // Si supera a la vida max, vida min es igualada a vida max.
				if( (this.stats[stat].min + stats[stat].min) > this.stats[stat].max ){
					...
					this.updateStats({ 'stats' : stat , 'value' : this.stats[stat] });
					continue;
				}
			}
	...
	*/
	...
	this.lifeEvent = function( params ){
		if( this.stats.life.min <=0 ){
			...
			console.log('Te mato ' + params.author.name )
		}
	}
	...
	*/
	...
	this.updateInventory = function(){
		SERVER.queue({
			...
			"data" : { "all" : [ { "three" : ['inventory'] , "value" : this.inventory } ] }
		});
	}
	...
	this.updateState = function( params ){
		...
		SERVER.queue({
			 "being" : this.name ,
			 "controls" : this.controls,
			 "data" : { "all" : [ { "three" : ['state',params.state] , "value" : this.state[ params.state ] } ] }
		});
	...
	this.updatePosition = function(){
		SERVER.queue({
			"being" : this.name ,
			"controls" : this.controls,
			"data" : { "all" : [ { "three" : ['position'] , "value" : this.position } ] }
		});
	}
	...
	this.updateTile = function( params ){// Actualiza objeto being/artifact en un tile dado.
		SERVER.queue({
			 "tile" : null ,
			 "data" : { 'type' : params.object.type , 'position' : params.object.position } ] }
		});
	...
	this.updateWorldObjects = function(){ // Actualiza objetos instanciados en el world.
		SERVER.queue({
			 "world" : null ,
			 "data" : { "three" : ['createdObjects'] , "value" : this }
		});
	}
